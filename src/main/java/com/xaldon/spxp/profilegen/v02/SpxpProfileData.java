...
public class SpxpProfileData {
    ...
    public void writePrivateData(File profilesDir) throws Exception {
        ...
        while(itFriendConnections.hasNext()) {
            ...
            JSONObject friendMembershipObj = Tools.newOrderPreservingJSONObject();
    ...
    public void writeSpxpProfile(File targetDir, File imageSourceDir) throws Exception {
        ...
        for(int i = 0; i < privateData.size(); i++) {
            ...
            if(!p.isEmpty()) {
                privateArray.put(SpxpCryptoToolsV02.encryptSymmetricCompact(p.toString(), keySpec.getKeyId(), keySpec.getSymmetricKey()));
            }
        }
    ...
    private JSONObject encryptProfilePhoto(String profilePhoto, File imageSourceDir, File targetDir) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IOException {
        ...
        // IV
        byte[] iv = SpxpCryptoToolsV02.generateSymmetricKey(ivLength);
        // algo params
        ...
        result.put("iv", CryptoTools.encodeBase64Url(iv));
        result.put("k", CryptoTools.encodeBase64Url(secretKey.getEncoded()));
        result.put("tag", CryptoTools.encodeBase64Url(tag));
            ...
            for(int i = 0; i < groups.size(); i++) {
                ...
                while(it.hasNext()) {
                    ...
                    for(SpxpRoundKey rk : groups.get(g).getRoundKeys()) {
                        ...
                        String encryptedSymmetricKey = SpxpCryptoToolsV02.encryptSymmetricCompact(jwkString, ks.getKeyId(), ks.getSymmetricKey());
                        if(!condensed) {
                            out.print("            ");
                        }
    ...
    */
    ...
    private JSONObject processPostEncryption(JSONObject obj, Date ts) throws Exception {
        ...
        JSONArray privateArray = new JSONArray();
        privateArray.put(new JSONObject(SpxpCryptoToolsV02.encryptSymmetricJson(obj.toString(), result.getString("timestampUTC"), receipients)));
